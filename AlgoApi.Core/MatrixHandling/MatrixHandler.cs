...
namespace AlgoApi.Core.MatrixHandling
{
    ...
    {
        public T[][] GetRandomMatrixOfEquitableRepeatedValues<T>(int repetition, T[] values)
        {
            var rand = new Random();
            try
            {
                ...
                // var randomList = values.GetRange(0, values.Count).SelectMany(i => Enumerable.Repeat(i, repetition))
                //     .ToList()
                //     .OrderBy(j => rand.Next()).ToList();
        ...
        public void DisplayMatrix<T>(T[][] matrix)
        {
        ...
        public int[][] GetMatrixMask(int maskVal)
        {
