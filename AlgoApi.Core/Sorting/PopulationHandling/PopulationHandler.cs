...
{
    public class PopulationHandler<T>: IPopulationHandler<T>
    {
        private VectorHandler<T> VectorHandler { get; set; } = new VectorHandler<T>();
        private IEnumerableStrategyByIndex EnumerableStrategyByIndex { get; set; } = new ArrayStrategyByIndex();
        private IErrorTester ErrorTester { get; set; } = new YPositionError();
        ...
        public int[][][] GeneratePopulation(List<TagVector<T>> vectors, int popSize)
        {
            ...
            {
                VectorHandler.ShuffleVectorsPositions(vectors);
        ...
        public Dictionary<int[][], int> EvaluatePopulation(int[][][] pop, List<TagVector<T>> vectors)
        {
            ...
            {
                VectorHandler.SetPositionsToVectors(entity, vectors);
        ...
        public int[][][] CrossBreed(int[][][] breeders, int popSize)
        {
            ...
            for (var i = 0; i < popSize; i++)
            {
                ...
                var idx2 = EnumerableStrategyByIndex.GetDistinctRandomIndex(breeders, idx1);
        ...
        public void MutatePop(int[][][] pop, float mutationRate)
        {
            ...
            foreach (var entity in pop)
            {
                ...
                var idx2 = EnumerableStrategyByIndex.GetDistinctRandomIndex(entity, idx1);
                EnumerableStrategyByIndex.SwapElement(entity, idx1, idx2);
            }
        }
