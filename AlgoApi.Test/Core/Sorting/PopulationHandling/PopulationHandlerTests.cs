...
namespace AlgoApi.Test.Services.Sorting.PopulationHandling
{
    [TestFixture]
    public class PopulationHandlerTests
    {
        ...
        {
            var populationHandler = new PopulationHandler<int>();
            const int popSize = 4;
        ...
        {
            var populationHandler = new PopulationHandler<string>();
            var pop = new []
            {
                new [] {new [] {0, 2}, new [] {2, 2}},
                new [] {new [] {1, 2}, new [] {1, 1}}
            ...
            Assert.True(expectedChildren.Any(expectedChild => expectedChild[0].SequenceEqual(child[0]) && expectedChild[1].SequenceEqual(child[1])));
        }
        ...
        public void EvaluatePopulation()
        {
            ...
            var pop = Enumerable.Range(0, 2).
                Select(i => Enumerable.Range(0, 4).
                        Select(l => new [] {l / 2 , l % 2}).ToArray()).
                ToArray();
            var tmp = pop[0][0];
            pop[0][0] = pop[0][3];
            pop[0][3] = tmp;
            ...
            for (var i = 0; i < asserts.Count; i++)
            {
                var pops = Enumerable.
                    ...
                    Select(c => Enumerable.
                        ...
                        Select(e => Enumerable.
                            Range(0, 4).
                            Select(p => new [] {p / 2, p % 2}).
                            ToArray()).
                        ToArray())
                    .ToList();
                
                populationHandler.MutatePop(pops[0], mutationRates[i]);
                asserts[i](Enumerable.
                    Range(0, pops[1].Length).
                    All(j => Enumerable.
                        Range(0, pops[1][j].Length).
                        All(k => pops[0][j][k].
                            SequenceEqual(pops[1][j][k])))
                );
            }
